...
const ManualForm = ({ currentTime, onSubmit, onClose }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="p-4 border-b border-amber-100 bg-white/60 rounded-lg space-y-3 text-sm">
            <div className="flex items-center justify-between mb-2">
                <span className="font-semibold text-gray-700">Add manual detection</span>
                <button type="button" onClick={onClose} className="p-1.5 rounded-md text-gray-400 hover:text-gray-600 hover:bg-amber-100 transition-colors" aria-label="Close">
                    <X className="w-4 h-4" />
                </button>
            </div>
            <div className="space-y-2">
                ...
                <input
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-amber-300"
                    value={type}
            ...
            <div className="flex items-center gap-2 pt-1">
                <Button type="submit" disabled={!type.trim()} size="sm" className="bg-red-700 hover:bg-red-800 text-white text-sm font-medium px-4">
                    Add detection
                </Button>
