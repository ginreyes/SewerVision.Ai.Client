...
import {
  ...
  Eye, AlertTriangle, Maximize2, Minimize2, RotateCcw,
} from "lucide-react";
...
const REVIEWS = [
  { id: "1", project: "PRJ-0087", frame: 1142, pacp: "BC-L", aiLabel: "Fine Crack Longitudinal", aiConfidence: 91, grade: "Grade 2", status: "pending" },
  { id: "2", project: "PRJ-0087", frame: 2351, pacp: "RIM", aiLabel: "Root Intrusion", aiConfidence: 78, grade: "Grade 3", status: "pending" },
  { id: "3", project: "PRJ-0088", frame: 445, pacp: "JOM", aiLabel: "Joint Offset", aiConfidence: 95, grade: "Grade 3", status: "approved" },
  { id: "4", project: "PRJ-0088", frame: 789, pacp: "DFV", aiLabel: "Deformed Pipe", aiConfidence: 62, grade: "Grade 4", status: "rejected" },
];
...
export default function ComparisonViewer() {
  ...
  function handleDecision(decision) {
    setReviews(prev => prev.map(r => r.id === selectedId ? { ...r, status: decision } : r));
    showAlert(decision === "approved" ? "Detection approved" : "Detection rejected", decision === "approved" ? "success" : "error");
    const next = pending.find(r => r.id !== selectedId);
        ...
        <div className="w-64 shrink-0 space-y-2">
          ...
          {reviews.map(r => (
            <button key={r.id} onClick={() => setSelectedId(r.id)}
              className={`w-full text-left p-3 rounded-xl border transition-all ${selectedId === r.id ? "border-rose-300 bg-rose-50" : "border-gray-200 bg-white hover:border-rose-200"}`}>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs font-mono text-gray-500">{r.project} #{r.frame}</span>
                {r.status === "approved" && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />}
                {r.status === "rejected" && <XCircle className="w-3.5 h-3.5 text-red-500" />}
                {r.status === "pending" && <AlertTriangle className="w-3.5 h-3.5 text-amber-400" />}
              </div>
              <p className="text-xs font-medium text-gray-900">{r.aiLabel}</p>
              <div className="flex items-center gap-1.5 mt-1">
                <Badge variant="outline" className={`text-[10px] ${GRADE_COLORS[r.grade]}`}>{r.grade}</Badge>
                <Badge variant="outline" className={`text-[10px] ${CONF_COLOR(r.aiConfidence)}`}>{r.aiConfidence}%</Badge>
              </div>
            </button>
          ))}
        </div>
            ...
            <>
              ...
              <Card className="border-gray-200">
                <CardContent className="p-3.5 flex items-center gap-4 flex-wrap">
                  <div><span className="text-xs text-gray-500">Project:</span> <span className="text-xs font-bold text-gray-900 ml-1">{review.project}</span></div>
                  <div><span className="text-xs text-gray-500">Frame:</span> <span className="text-xs font-bold text-gray-900 ml-1">#{review.frame}</span></div>
                  <div><span className="text-xs text-gray-500">AI Label:</span> <span className="text-xs font-bold text-gray-900 ml-1">{review.aiLabel}</span></div>
                  <Badge variant="outline" className={`text-[10px] ${CONF_COLOR(review.aiConfidence)}`}>AI Confidence: {review.aiConfidence}%</Badge>
                  <Badge variant="outline" className={`text-[10px] ${GRADE_COLORS[review.grade]}`}>{review.grade}</Badge>
                  <Badge variant="outline" className={`text-[10px] capitalize ${review.status === "approved" ? "bg-emerald-50 text-emerald-700" : review.status === "rejected" ? "bg-red-50 text-red-700" : "bg-amber-50 text-amber-700"}`}>{review.status}</Badge>
                </CardContent>
              </Card>
              ...
              <Card className="border-gray-200">
                <CardContent className="p-3.5">
                  ...
                  {review.status === "pending" && (
                    <div className="flex items-center gap-3">
                      <Button onClick={() => handleDecision("approved")} className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white gap-1.5">
                        <CheckCircle2 className="w-4 h-4" /> Approve
                      </Button>
                      <Button onClick={() => handleDecision("rejected")} className="flex-1 bg-red-500 hover:bg-red-600 text-white gap-1.5">
