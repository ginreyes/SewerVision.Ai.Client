...
const UserPage = () => {
  ...
  // Mutation hooks — each invalidates the users list on success so refetch
  // is handled by TanStack rather than manual refetch() calls.
  const deleteUserMutation = useDeleteUser();
  const updateUserInfoMutation = useUpdateUserInfo();
  const sendEmailMutation = useSendEmailToUser();
    ...
    showDelete({
      ...
      onConfirm: async () => {
        try {
          ...
          await deleteUserMutation.mutateAsync({ user_id, actorUsername, actorRole });
  ...
  const handleDisable = async (item) => {
    ...
    try {
      await updateUserInfoMutation.mutateAsync({ user_id: userId, active: newActive });
  ...
  const handleSendEmail = async (subject, message) => {
    ...
    try {
      ...
      showAlert(err?.message || "Error sending email", "error");
    }
  };
  ...
  const handleChangePassword = async (newPassword) => {
    ...
    // Mutation throws on failure; reaching here = success
        ...
        try {
          await Promise.all(
            selectedUsers.map((userId) =>
              updateUserInfoMutation.mutateAsync({ user_id: userId, active: false })
            )
          );
          showAlert(`Successfully disabled ${selectedUsers.length} user(s)`, "success");
          setSelectedUsers([]);
        } catch (err) {
          showAlert(err?.message || "Failed to disable users", "error");
        }
      },
      onCancel: () => showAlert("Cancelled", "info"),
    });
    ...
    showDelete({
      ...
      onConfirm: async () => {
        try {
          await Promise.all(
            selectedUsers.map((userId) =>
              deleteUserMutation.mutateAsync({ user_id: userId })
            )
          );
          showAlert(`Successfully deleted ${selectedUsers.length} user(s)`, "success");
          setSelectedUsers([]);
        } catch (err) {
          showAlert(err?.message || "Failed to delete users", "error");
        }
      },
      onCancel: () => showAlert("Cancelled", "info"),
    });
  ...
  // Stats for the Users tab — derived from the loaded users page (matches
  // the dashboard card style). totalUsers comes from server pagination.
  const usersTabStats = useMemo(() => {
    const active = users.filter((u) => u.status === "Active" || u.active === true).length;
    const inactive = users.filter((u) => u.status === "Inactive" || u.active === false).length;
    const admins = users.filter((u) => (u.role || "").toLowerCase() === "admin").length;
    return { total: totalUsers || users.length, active, inactive, admins };
  }, [users, totalUsers]);
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white truncate">{pageTitle}</h1>
            <p className="text-sm text-gray-500 dark:!text-gray-300 mt-0.5">{pageDescription}</p>
          </div>
        </div>
        {activeTab === "users" && (
      ...
      {/* Stats Cards — dashboard style for the users tab, legacy CardList for others */}
      {activeTab === "users" ? (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <GenericStatCard
            icon={Users}
            value={usersTabStats.total}
            label="Total Users"
          ...
          <GenericStatCard
            icon={Shield}
            value={usersTabStats.admins}
            label="Admins"
            subtitle="Full-access accounts"
            color="purple"
          />
        </div>
      ) : (
      ...
      <div className="bg-white dark:bg-[#0c0c0e] rounded-xl shadow-sm border border-gray-100 dark:border-[#27272a]">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <div className="border-b border-gray-100 dark:border-[#27272a] px-6 pt-4">
            ...
            <TabsContent value="users" className="p-0 m-0">
              {selectedUsers.length > 0 && (
                <div className="bg-blue-50 dark:bg-blue-500/10 border-b border-blue-200 dark:border-blue-500/30 px-6 py-4">
                  <div className="flex items-center justify-between flex-wrap gap-3">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      <Button size="sm" variant="outline" onClick={handleExportSelected} className="gap-2 h-9 text-xs">
                        <Download className="w-4 h-4" /> Export
                      </Button>
                      <Button size="sm" variant="outline" onClick={handleBulkEmail} className="gap-2 h-9 text-xs">
                        <Mail className="w-4 h-4" /> Email All
                      </Button>
                      <Button size="sm" variant="outline" onClick={handleBulkDisable} className="gap-2 h-9 text-xs">
